function [Vseq,Dseq,Wseq] = eigenshuffle_async(Aseq,Bseq,O1,O2)
...
%
% Arguments: (Output)
%  Vseq - a 3-d array (pxpxn1xn2) of eigenvectors. Each
...
if (Asize(1)~=Asize(2))
    error('Aseq must be a (pxpxn1xn2) array of eigen-problems, each of size pxp')
end
if  (Bsize(1)~=Bsize(2))
	error('Bseq must be a (pxpxn1xn2) array of eigen-problems, each of size pxp')
end
if (Asize(1)~=Bsize(1))
	error('Aseq and Bsequences must both be (pxpxn1x2) arrays of the same size p')
end
if (Asize(3)~=Bsize(3))
    error('Aseq and Bsequences must both be (pxpxn1xn2) arrays with the same number of eigen-problems n')
end
if (Asize(4)~=Bsize(4))
    error('Aseq and Bsequences must both be (pxpxn1xn2) arrays with the same number of eigen-problems n')
end
...
Vseq = zeros(p,p,n1,n2);
Wseq = zeros(p,p,n1,n2);
Dseq = zeros(p,n1,n2);
for i = 1:n1
    for j = 1:n2
      [V,D,W] = eig(Aseq(:,:,i,j),Bseq(:,:,i,j),'vector');
      % initial ordering is purely in decreasing order.
      % If any are complex, the sort is in terms of the
      % real part.
      [~,iSort] = sort(real(D),1,'descend');

      [V,W] = mkorthog(Bseq(:,:,i,j),V,W);

      chkorthog(Aseq(:,:,i,j),Bseq(:,:,i,j),V,D,W)
...
for j = 1:n2
    for i = 2:n1
      ...
      if i > 2
          dVdO = (Vseq(:,:,i-1,j) - Vseq(:,:,i-2,j)) / (O1(i-1) - O1(i-2) + eps);
          dDdO = (Dseq(:,i-1,j)   - Dseq(:,i-2,j)  ) / (O1(i-1) - O1(i-2) + eps);
          dWdO = (Wseq(:,:,i-1,j) - Wseq(:,:,i-2,j)) / (O1(i-1) - O1(i-2) + eps);
      else
          dVdO = zeros(p,p);
          dDdO = zeros(p,1);
          dWdO = zeros(p,p);
      ...
      V1 = Vseq(:,:,i-1,j) + dVdO * (O1(i) - O1(i-1));
      D1 = Dseq(:,i-1,j)   + dDdO * (O1(i) - O1(i-1));
      W1 = Wseq(:,:,i-1,j) + dWdO * (O1(i) - O1(i-1));

      %Penalise errors in both the eigenvectors and -values
      dist_vec = abs(1-W1'*Bseq(:,:,i,j)*V2);
      ...
      if ~isempty(P)
       ...
       chkorthog(Aseq(:,:,i,j),Bseq(:,:,i,j),Vseq(:,:,i,j),Dseq(:,i,j),Wseq(:,:,i,j));
    end
end
...
for j = 2:n2
    ...
    %now predict the new W and D by extrapolating from the old ones
    if j > 2
        dVdO = (Vseq(:,:,i,j-1) - Vseq(:,:,i,j-2)) / (O2(j-1) - O2(j-2) + eps);
        dDdO = (Dseq(:,i,j-1)   - Dseq(:,i,j-2)  ) / (O2(j-1) - O2(j-2) + eps);
        dWdO = (Wseq(:,:,i,j-1) - Wseq(:,:,i,j-2)) / (O2(j-1) - O2(j-2) + eps);
    else
        dVdO = zeros(p,p);
        dDdO = zeros(p,1);
        dWdO = zeros(p,p);
    ...
    V1 = Vseq(:,:,i,j-1) + dVdO * (O2(j) - O2(j-1));
    D1 = Dseq(:,i,j-1)   + dDdO * (O2(j) - O2(j-1));
    W1 = Wseq(:,:,i,j-1) + dWdO * (O2(j) - O2(j-1));
    ...
    Vseq(:,:,:,j) = Vseq(:,reorder,:,j);
    Dseq(:,:,j) = Dseq(reorder,:,j);
    Wseq(:,:,:,j) = Wseq(:,reorder,:,j);
...
%now they're all aligned in the O1 direction
%now align each in the O2 direction

function chkorthog(A,B,V,D,W)
err = diag(W'*B*V) - 1;
% assert(all(err<1E-6));

err = (diag(W'*A*V) - D)./(abs(D)+eps);
% assert(all(err(~isinf(D))<1E-6));
